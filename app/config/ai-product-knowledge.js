...
Recommends products that pair with what a customer is already looking at, usually placed near the product page's Add-to-Cart button (it's its own widget, not part of the cart drawer). Settings: on/off, a template (fbt1 Classic Grid, fbt2 Modern Cards, fbt3 Vertical List), manual or AI-picked pairings, layout (carousel/grid/vertical), interaction type (classic, bundle, quick-add, checkbox+quantity), colors, whether prices and an "add all" button show. Rules pair specific trigger product(s) with offer products — omit the trigger to apply a rule to every product. Example: "when someone views the Yoga Mat, recommend the Yoga Block and Strap." A per-rule discount value CAN be stored, but it is not currently applied anywhere — the storefront widget and the admin FBT page both ignore it — so never tell a merchant an FBT discount is active or was applied; if asked for one, say plainly it isn't supported yet. An existing rule's trigger/offer products can be edited in place (adding, removing, or replacing products without losing the rule's id or discount setting) — removing and recreating a rule is only needed to delete it outright.
