import cartEditorStyles from '../styles/cart-editor.css?url';
import { useCurrency } from '../components/CurrencyContext';
import AiUpsellSection from '../components/AiUpsellSection';
import React, { useState, useEffect } from 'react';
import {
  ArrowLeftIcon, SettingsIcon, ChartVerticalIcon,
  ColorIcon, DiscountCodeIcon, ProductIcon, CartIcon, CodeIcon,
  LayoutHeaderIcon, MegaphoneIcon, CashDollarIcon,
  ChevronDownIcon, DesktopIcon, MobileIcon,
} from '@shopify/polaris-icons';

export const links = () => [{ rel: 'stylesheet', href: cartEditorStyles }];
import {
  Page,
  Layout,
  Card,
...
export default function CartDrawerAdmin() {
  ...
  const [cartWidth, setCartWidth] = useState('normal');
  const [cartBorderRadius, setCartBorderRadius] = useState(8);
  const [cartShadow, setCartShadow] = useState(true);
  const [cartAnimation, setCartAnimation] = useState('slide');

  // General section state
  const [openOnAdd, setOpenOnAdd] = useState(true);
  const [openOnIconClick, setOpenOnIconClick] = useState(true);
  ...
  const [headerTitle, setHeaderTitle] = useState('Your Cart');
  const [headerCloseStyle, setHeaderCloseStyle] = useState('icon');
  const [headerBgColor, setHeaderBgColor] = useState('#ffffff');
  const [headerTextColor, setHeaderTextColor] = useState('#1a1a1a');
  const [headerBorderBottom, setHeaderBorderBottom] = useState(true);

  // Announcements section state
  const [announcementsText, setAnnouncementsText] = useState('Free shipping on orders over ₹999!');
  const [announcementsBgColor, setAnnouncementsBgColor] = useState('#4f46e5');
  const [announcementsTextColor, setAnnouncementsTextColor] = useState('#ffffff');
  const [announcementsFontSize, setAnnouncementsFontSize] = useState(14);

  // Empty Cart section state
  const [emptyCartMessage, setEmptyCartMessage] = useState('Your cart is empty');
  const [emptyCartShowContinue, setEmptyCartShowContinue] = useState(true);
  const [emptyCartShowRecs, setEmptyCartShowRecs] = useState(true);
  ...
  const renderLeftSidebar = () => {
    const ACCORDION_GROUPS = [
      {
        title: 'Settings',
        items: [
          { id: 'design',   label: 'Design',   icon: ColorIcon    },
      ...
      },
      {
        title: 'Header',
        items: [
          { id: 'header', label: 'Header Style', icon: LayoutHeaderIcon },
        ],
      },
      {
        title: 'Body',
        items: [
          { id: 'announcements', label: 'Announcements',  icon: MegaphoneIcon,    toggleKey: 'announcementsEnabled' },
          { id: 'progress-bar',  label: 'Progress Bar',   icon: ChartVerticalIcon,toggleKey: 'progressBarEnabled'   },
          { id: 'coupon',        label: 'Coupon Slider',  icon: DiscountCodeIcon, toggleKey: 'couponSliderEnabled'  },
          { id: 'upsell',        label: 'Upsell Products',icon: ProductIcon,      toggleKey: 'upsellEnabled'        },
          { id: 'empty-cart',    label: 'Empty Cart',     icon: CartIcon                                            },
        ],
      },
      {
        title: 'Footer',
        items: [
          { id: 'checkout',   label: 'Checkout Button', icon: CashDollarIcon },
          { id: 'custom-css', label: 'Custom CSS',      icon: CodeIcon       },
        ],
      },
    ];
      ...
      <div className="cart-editor-left">
        ...
        <div className="cart-editor-left-header">
          <div className="cart-editor-left-header-top">
            <button className="cart-editor-back-btn" onClick={() => window.history.back()}>
          ...
          <div className="cart-editor-left-header-controls">
            <div className="cart-editor-control-row">
              ...
              <button
                className={`cart-editor-status-pill ${cartStatus ? 'active' : ''}`}
                onClick={() => {
                  if (cartStatus) {
                    setShowDeactivateModal(true);
                  } else {
            ...
            <div className="cart-editor-control-row">
              ...
              <div className="cart-editor-segmented">
                <button
                  className={`segmented-btn ${previewCartState[0] === 'items' ? 'active' : ''}`}
                  onClick={() => setPreviewCartState(['items'])}
                >Items</button>
                <button
                  className={`segmented-btn ${previewCartState[0] === 'empty' ? 'active' : ''}`}
          ...
          {ACCORDION_GROUPS.map((group) => (
            <div key={group.title} className="accordion-group">
              ...
              {group.items.map((item) => {
                ...
                const isOn = item.toggleKey != null ? featureStates[item.toggleKey] : null;
                return (
                  <div key={item.id} className={`accordion-item ${isSelected ? 'open' : ''}`}>
                    <button
                      className="accordion-row"
                      onClick={() => {
                        const nextTab = isSelected ? null : item.id;
                        setSelectedTab(nextTab);
                        setPreviewCartState([nextTab === 'empty-cart' ? 'empty' : 'items']);
                      }}
                    >
                      <span className="accordion-row-icon"><Icon source={item.icon} /></span>
                      <span className="accordion-row-label">{item.label}</span>
                      {isOn !== null && (
                        <span className={`accordion-badge ${isOn ? 'on' : 'off'}`}>
                          {isOn ? 'ON' : 'OFF'}
                        </span>
                      )}
                      <span className={`accordion-chevron ${isSelected ? 'open' : ''}`}>
    ...
    // ── OLD CARD-BASED SIDEBAR (kept below as dead code for reference) ──
    return (
  ...
  // ── Inline ColorField matching Solaris ColorField.tsx ──────────────────── */
  const SolarisColorField = ({ label, value, onChange }) => (
    <div style={{ flex: 1 }}>
      <Text as="p" variant="bodySm" fontWeight="semibold">{label}</Text>
      <div style={{ marginTop: '6px', display: 'flex', alignItems: 'center', gap: '8px' }}>
        <div style={{ width: '32px', height: '32px', borderRadius: '6px', border: '1px solid #c9cccf', overflow: 'hidden', flexShrink: 0, position: 'relative', cursor: 'pointer' }}>
        ...
        <TextField label="" labelHidden value={value} onChange={onChange} autoComplete="off"
          prefix={<div style={{ width: '14px', height: '14px', borderRadius: '3px', background: value, border: '1px solid #c9cccf', flexShrink: 0 }} />}
        />
      </div>
    </div>
  );

  // ── Inline FeatureToggle ─────────────────────────────────────────────── */
  const FeatureToggle = ({ enabled, onToggle }) => (
    <button
      onClick={() => onToggle(!enabled)}
      style={{ width: '48px', height: '26px', borderRadius: '13px', border: 'none', background: enabled ? '#008060' : '#babec3', position: 'relative', cursor: 'pointer', transition: 'background 0.2s', flexShrink: 0, padding: 0 }}
    >
      <span style={{ position: 'absolute', top: '3px', left: enabled ? '25px' : '3px', width: '20px', height: '20px', borderRadius: '50%', background: '#fff', transition: 'left 0.2s', boxShadow: '0 1px 3px rgba(0,0,0,0.25)', display: 'block' }} />
    </button>
  );
  ...
  const renderEditorPanel = () => {
    ...
    if (selectedTab === 'design') {
      return (
        <div style={{ padding: '24px', height: '100%', overflowY: 'auto', backgroundColor: '#fafbfc' }}>
          <BlockStack gap="500">
            <BlockStack gap="100">
              <Text as="h1" variant="heading2xl" fontWeight="bold">Design</Text>
            ...
            <Card>
              <BlockStack gap="400">
                <Select
                  label="Cart Width"
                  options={[{ label: 'Slim', value: 'slim' }, { label: 'Normal', value: 'normal' }, { label: 'Thick', value: 'thick' }]}
                  value={cartWidth}
                  onChange={setCartWidth}
                />
                <RangeSlider label={`Border Radius: ${cartBorderRadius}px`} value={cartBorderRadius} min={0} max={24} output onChange={(v) => setCartBorderRadius(v)} />
                <Checkbox label="Drop Shadow" checked={cartShadow} onChange={setCartShadow} />
                <Select
                  label="Open Animation"
                  options={[{ label: 'Slide', value: 'slide' }, { label: 'Fade', value: 'fade' }, { label: 'None', value: 'none' }]}
                  value={cartAnimation}
                  onChange={setCartAnimation}
                />
              </BlockStack>
            </Card>
          </BlockStack>
    ...
    if (selectedTab === 'general') {
      return (
        <div style={{ padding: '24px', height: '100%', overflowY: 'auto', backgroundColor: '#fafbfc' }}>
          <BlockStack gap="500">
            <BlockStack gap="100">
              <Text as="h1" variant="heading2xl" fontWeight="bold">General Settings</Text>
              <Text as="p" variant="bodyMd" tone="subdued">Configure how the cart drawer behaves on your store.</Text>
            </BlockStack>
            <Card>
              <BlockStack gap="400">
                <Checkbox label="Open Cart Drawer when an item is added to the cart" checked={openOnAdd} onChange={setOpenOnAdd} />
                <Checkbox label="Open Cart Drawer when the cart icon is clicked" checked={openOnIconClick} onChange={setOpenOnIconClick} />
                <Checkbox label='Show "Continue shopping" button' checked={showContinueShopping} onChange={setShowContinueShopping} />
                <Select
                  label="Cart Drawer Position"
                  options={[{ label: 'Right side', value: 'right' }, { label: 'Left side', value: 'left' }]}
                  value={cartPosition}
    ...
    if (selectedTab === 'header') {
      return (
        <div style={{ padding: '24px', height: '100%', overflowY: 'auto', backgroundColor: '#fafbfc' }}>
          <BlockStack gap="500">
            <BlockStack gap="100">
              <Text as="h1" variant="heading2xl" fontWeight="bold">Header</Text>
              <Text as="p" variant="bodyMd" tone="subdued">Customize the cart drawer header appearance.</Text>
            </BlockStack>
            <Card>
              <BlockStack gap="400">
                <TextField label="Cart Title" value={headerTitle} onChange={setHeaderTitle} autoComplete="off" />
                <Select
                  label="Close Button Style"
                  options={[{ label: 'Icon (X)', value: 'icon' }, { label: 'Text (Close)', value: 'text' }]}
                  value={headerCloseStyle}
                  onChange={setHeaderCloseStyle}
                />
                <InlineStack gap="400">
                  <SolarisColorField label="Background Color" value={headerBgColor} onChange={setHeaderBgColor} />
                  <SolarisColorField label="Text Color" value={headerTextColor} onChange={setHeaderTextColor} />
                </InlineStack>
                <Checkbox label="Show bottom border" checked={headerBorderBottom} onChange={setHeaderBorderBottom} />
              </BlockStack>
            </Card>
          </BlockStack>
        </div>
      ...
      return (
        <div style={{ padding: '24px', height: '100%', overflowY: 'auto', backgroundColor: '#fafbfc' }}>
          <BlockStack gap="500">
            <InlineStack align="space-between" blockAlign="center">
              <BlockStack gap="100">
                <Text as="h1" variant="heading2xl" fontWeight="bold">Announcements</Text>
                <Text as="p" variant="bodyMd" tone="subdued">Display a promotional banner at the top of your cart drawer.</Text>
              </BlockStack>
              <FeatureToggle
                enabled={featureStates.announcementsEnabled}
                onToggle={(v) => setFeatureStates(s => ({ ...s, announcementsEnabled: v }))}
              />
            </InlineStack>
            {featureStates.announcementsEnabled && (
              <Card>
                <BlockStack gap="400">
                  <TextField label="Announcement Text" value={announcementsText} onChange={setAnnouncementsText} autoComplete="off" multiline={2} />
                  <InlineStack gap="400">
                    <SolarisColorField label="Background Color" value={announcementsBgColor} onChange={setAnnouncementsBgColor} />
                    <SolarisColorField label="Text Color" value={announcementsTextColor} onChange={setAnnouncementsTextColor} />
                  </InlineStack>
                  <RangeSlider label={`Font Size: ${announcementsFontSize}px`} value={announcementsFontSize} min={10} max={20} output onChange={(v) => setAnnouncementsFontSize(v)} />
                </BlockStack>
              </Card>
            )}
          </BlockStack>
    ...
    // ── Empty Cart section ───────────────────────────────────────────────── */
    if (selectedTab === 'empty-cart') {
      return (
        <div style={{ padding: '24px', height: '100%', overflowY: 'auto', backgroundColor: '#fafbfc' }}>
          <BlockStack gap="500">
            <BlockStack gap="100">
              <Text as="h1" variant="heading2xl" fontWeight="bold">Empty Cart</Text>
              <Text as="p" variant="bodyMd" tone="subdued">Configure what customers see when their cart is empty.</Text>
            </BlockStack>
            <Card>
              <BlockStack gap="400">
                <TextField label="Empty Cart Message" value={emptyCartMessage} onChange={setEmptyCartMessage} autoComplete="off" />
                <Checkbox label='Show "Continue shopping" button' checked={emptyCartShowContinue} onChange={setEmptyCartShowContinue} />
                <Checkbox label="Show recommended products" checked={emptyCartShowRecs} onChange={setEmptyCartShowRecs} />
              </BlockStack>
            </Card>
          </BlockStack>
        </div>
    ...
    // ── Checkout Button — new dedicated section ──────────────────────────── */
    if (selectedTab === 'checkout') {
      return (
        <div style={{ padding: '24px', height: '100%', overflowY: 'auto', backgroundColor: '#fafbfc' }}>
          <BlockStack gap="500">
            <BlockStack gap="100">
              <Text as="h1" variant="heading2xl" fontWeight="bold">Checkout Button</Text>
              <Text as="p" variant="bodyMd" tone="subdued">Customize the checkout button appearance.</Text>
            </BlockStack>
            <Card>
              <BlockStack gap="400">
                <TextField label="Checkout Button Text" value={checkoutName} onChange={setCheckoutName} autoComplete="off" />
                <TextField label="Footer Text" value={checkoutFooterText} onChange={setCheckoutFooterText} autoComplete="off" helpText="Text displayed below the checkout button" />
              </BlockStack>
            </Card>
            <Card>
              <BlockStack gap="400">
                ...
                <InlineStack gap="400">
                  <SolarisColorField label="Background Color" value={checkoutButtonStyle.backgroundColor} onChange={(v) => setCheckoutButtonStyle(s => ({ ...s, backgroundColor: v }))} />
                  <SolarisColorField label="Text Color" value={checkoutButtonStyle.textColor} onChange={(v) => setCheckoutButtonStyle(s => ({ ...s, textColor: v }))} />
                </InlineStack>
                <RangeSlider label={`Border Radius: ${checkoutButtonStyle.borderRadius}px`} value={checkoutButtonStyle.borderRadius} min={0} max={24} output onChange={(v) => setCheckoutButtonStyle(s => ({ ...s, borderRadius: v }))} />
              </BlockStack>
            </Card>
          </BlockStack>
        </div>
    ...
    // ── Custom CSS — new dedicated section ───────────────────────────────── */
    if (selectedTab === 'custom-css') {
      return (
        <div style={{ padding: '24px', height: '100%', overflowY: 'auto', backgroundColor: '#fafbfc' }}>
          <BlockStack gap="500">
            <BlockStack gap="100">
              <Text as="h1" variant="heading2xl" fontWeight="bold">Custom CSS</Text>
              <Text as="p" variant="bodyMd" tone="subdued">Add custom CSS to override default cart drawer styles.</Text>
            </BlockStack>
            <Card>
              <BlockStack gap="300">
                <textarea
                  ...
                  style={{ width: '100%', minHeight: '240px', padding: '12px', fontFamily: 'monospace', fontSize: '13px', border: '1px solid #c9cccf', borderRadius: '8px', resize: 'vertical', lineHeight: 1.6, background: '#1e1e1e', color: '#d4d4d4' }}
                />
      ...
      <div className="cart-editor-root">
        ...
        <div className="cart-editor-right">
          <div className="cart-preview-stage-header">
            ...
            <div className="cart-editor-segmented">
              <button className="segmented-btn active"><Icon source={DesktopIcon} /></button>
              <button className="segmented-btn"><Icon source={MobileIcon} /></button>
            </div>
          </div>
