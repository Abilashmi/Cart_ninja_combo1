...
export async function applyOrderDelta(shop, dateStr, revenueDelta, orderCountDelta) {
  const db = getDb();
  await ensureAnalyticsTables(db);
  await db.execute(
    `INSERT INTO analytics_daily_rollup (shop_domain, date, revenue, order_count)
     VALUES (?, ?, ?, ?)
     ON DUPLICATE KEY UPDATE
       revenue = revenue + VALUES(revenue),
       order_count = order_count + VALUES(order_count)`,
    [shop, dateStr, revenueDelta, orderCountDelta]
  );
}
  ...
  for (const [date, m] of byDate.entries()) {
    await db.execute(
      `INSERT INTO analytics_daily_rollup
         (shop_domain, date, revenue, order_count, upsell_revenue, coupon_applied_count,
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE
         revenue = VALUES(revenue), order_count = VALUES(order_count),
      ...
      [
        shopDomain, date, m.revenue, m.order_count, m.upsell_revenue, m.coupon_applied_count,
