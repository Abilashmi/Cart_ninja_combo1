...
export async function loader({ request }) {
  ...
  const result = await callPhp('upsell_settings.php', 'GET', session.shop);
...
export async function action({ request }) {
  ...
  const result = await callPhp('upsell_settings.php', 'POST', session.shop, { shop: session.shop, ...body });
  return Response.json({ success: result.status === 'success', data: result.data });
}
