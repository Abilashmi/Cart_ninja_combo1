  ...
  try {
    ...
    await db.execute(
      ...
      [
        ...
        payload.updated_at ? new Date(payload.updated_at).toISOString().slice(0, 19).replace("T", " ") : null,
      ]
    );

    const dateStr = new Date().toISOString().slice(0, 10);
    await applyCartActivityDelta(shop, dateStr, "cart_update_count", 1);
  } catch (error) {
    console.error("[Webhook carts/update] Failed to record cart activity:", error.message);
  }
